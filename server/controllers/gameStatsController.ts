...
export const playerGameByGameStats = async (req: Request, res: Response, next: NextFunction) => {
    if (req.params.id) {
        ...
        if (player) {
            const stats = await getPlayerGameStats(player.id.toString(), req.params.season ? req.params.season : '20192020', player.position === 'Goalie');
            ...
            res.send({20182019: statsLastSeason, 20192020: stats});
        } else {
